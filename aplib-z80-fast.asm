...
_write_buffer:
  ; Set the address
  ld a, d
  out (VDP_ADDRESS_PORT), d
  ld a, e
  out (VDP_ADDRESS_PORT), e
  push hl
    push bc
      ; Write to it
      ld hl, BUFFER
      ...
      otir
    pop bc
    ; Subtract BUFFER_SIZE from bc - which is likely to overflow at the end
    ld hl, -BUFFER_SIZE
    add hl, bc
  pop hl
  ret
...
_ldir_vram_to_vram:
  ...
  ex af, af'
    ...
    push hl
      xor a
      sbc hl,de ; now hl = distance
      sbc hl,bc ; compare to count
      jr nc,+
      ; carry means count > distance
...
-:  ; Read into our buffer
    ...
    call _write_buffer
    jr nc,-
    ; Now we've written approximately distance bytes (maybe more) and we need to 

    ; no carry means no overlap
    ; TODO
